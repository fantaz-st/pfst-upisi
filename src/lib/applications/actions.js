...
import { emailUObradi } from "@/lib/email/emailUObradi";
...
export async function submitApplication(formData, slug, force = false) {
  ...
  const {
    first_name, last_name, email, phone, oib, birth_date, birth_place,
    gender, marital_status, citizenship, address, city, postal_code,
    program, study_type,
    father_name, father_occupation, father_address,
    mother_name, mother_occupation, mother_address,
    previous_institution, previous_program, previous_completion_year,
    other_education, ranking_score, enrollment_type,
  } = parsed.data;
  ...
  const { data: application, error: insertError } = await supabase
    ...
    .insert({
      ...
      citizenship, address, city, postal_code,
      program, study_type,
  ...
  for (const { documentType, file } of files) {
    ...
    if (documentType === "photo") {
      ...
      if (existing?.length) {
        ...
        await supabase.from("application_documents").delete()
          .eq("application_id", applicationId)
          .eq("document_type", "photo");
      }
    }
...
export async function updateApplicationStatus(applicationId, newStatus, adminMessage = null) {
  ...
  const { data: application } = await supabase
    .from("applications")
    .select("*, intakes ( title, academic_year )")
    .eq("id", applicationId)
    .single();
  ...
  try {
    if (newStatus === "needs_update" && adminMessage) {
      ...
      const template = emailPotrebneIzmjene({
        ime: application.first_name, prezime: application.last_name,
        brojPrijave: application.application_number,
        poruka: adminMessage, magicLink,
      });
      ...
      const template = emailUObradi({
        ime: application.first_name, prezime: application.last_name,
        brojPrijave: application.application_number,
        studij: programLabel, akademskaGodina,
      });
      await sendEmail({ to: application.email, ...template });
...
export async function updateApplicationViaToken(token, data) {
  ...
  try {
    const template = emailPotvrda({
      ime: parsed.data.first_name, prezime: parsed.data.last_name,
      brojPrijave: application.application_number,
...
export async function addApplicationNote(applicationId, note, adminId) {
  ...
  const { error } = await supabase.from("application_notes").insert({
    application_id: applicationId, note, admin_id: adminId,
  });
  if (error) return { error: error.message };
  return { success: true };
}
...
export async function requestEditLinkForExisting(applicationId) {
  ...
  const { data: application } = await supabase
    .from("applications")
    .select("id, email, first_name, last_name, application_number")
    .eq("id", applicationId)
    .single();
